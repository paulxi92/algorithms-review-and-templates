...
class Solution {
    ...
    private int helper(TreeNode node, long preSum, long sum) {
        if (node == null) {
            return 0;
        }
        long curSum = preSum + node.val;
        int res = m.getOrDefault(curSum - sum, 0);
        m.put(curSum, m.getOrDefault(curSum, 0) + 1);
        res += helper(node.left, curSum, sum);
        res += helper(node.right, curSum, sum);
