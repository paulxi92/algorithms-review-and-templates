...
// sol1: dfs with memory
// class Solution {
//     private int[][] mem; // mem[i][j]: max length starting with matrix[i][j]
...
//     private int helper(int r, int c, int[][] matrix) {
//         if (mem[r][c] != 0) {
//             return mem[r][c];
//         }
//         int m = matrix.length, n = matrix[0].length;
...
//                 continue;
//             }
//             res = Math.max(res, helper(x, y, matrix) + 1);
//         }
//         return mem[r][c] = res;
//     }
// }
...
class Solution {
    public int longestIncreasingPath(int[][] matrix) {
        ...
        int m = matrix.length, n = matrix[0].length;
        int[][] mem = new int[m][n]; // mem[i][j]: max length ending with matrix[i][j]
        int[][] dirs = {{0, -1}, {-1, 0}, {0, 1}, {1, 0}};
        int res = 1;
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
