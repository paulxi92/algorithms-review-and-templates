...
//         int[][] sums = new int[m][n]; // sums[i][j]: sum from matrix[0][0] to matrix[i][j]
//         for (int i = 0; i < m; ++i) {
//             for (int j = 0; j < n; ++j) {
//                 int t = matrix[i][j];
//                 if (i > 0) {
...
// sol2: treeset, binary search (use cumulative sum)
class Solution {
    public int maxSumSubmatrix(int[][] matrix, int k) {
        int m = matrix.length, n = matrix[0].length;
        int res = Integer.MIN_VALUE;
        for (int i = 0; i < n; ++i) { // if rows are larger than cols O(n^2*m*logm) < O(m^2*n*logn)
            int[] sums = new int[m]; // row cumulative sums
            for (int j = i; j < n; ++j) {
                for (int r = 0; r < m; ++r) {
                    sums[r] += matrix[r][j];
                }
                ...
                for (int sum : sums) {
                    curSum += sum;
                    Integer tmp = st.ceiling(curSum - k); // find start so that sums[end]-sums[start] <= k
                    if (tmp != null) {
