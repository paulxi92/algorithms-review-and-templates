...
Given an array of **unique** strings `words`, return _all the_ [word squares](https://en.wikipedia.org/wiki/Word\_square) _you can build from_ `words`. You can return the answer in **any order**.
...
class Solution {
    ...
    private void helper(int row, String[] words) {
        int n = words[0].length();
        if (row == n) {
        ...
        for (int i = 0; i < row; ++i) {
            prefix.append(out.get(i).charAt(row));
        }
        TrieNode cur = root;
