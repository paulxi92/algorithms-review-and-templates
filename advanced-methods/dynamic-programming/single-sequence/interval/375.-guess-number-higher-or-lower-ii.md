...
```java
// sol1: backtracking with memory, minmax (min of max loss)
// class Solution {
//     private int[][] mem;

//     public int getMoneyAmount(int n) {
//         mem = new int[n + 1][n + 1]; // mem[i][j]: min of max loss for guessing from i to j
//         for (int i = 1; i <= n; ++i) {
//             Arrays.fill(mem[i], -1);
//         }
//         return helper(1, n);
...
//         }
//         int res = Math.min(start + helper(start + 1, end), end + helper(start, end - 1));
...
// sol2: 2D dp (by range length), minmax (min of max loss)
class Solution {
    private int[][] mem;

    public int getMoneyAmount(int n) {
        int[][] dp = new int[n + 1][n + 1]; // dp[i][j]: min of max loss for guessing from i to j
        for (int len = 1; len < n; ++len) {
